...
create or replace function public.get_wrong_penalty_divisor(exam_type text)
returns numeric
language plpgsql
immutable
as $$
begin
  case upper(exam_type)
...
create table if not exists public.questions (
  ...
  exam_type text not null check (exam_type in ('YKS', 'LGS', 'KPSS', 'ALES')),
  lesson text not null,
  topic text not null,
  subtopic text,
  difficulty text not null check (difficulty in ('easy', 'medium', 'hard')),
  question_type text not null default 'multiple_choice' check (
    question_type in ('multiple_choice', 'mini_test', 'branch_mock', 'full_mock')
  ),
  source_type text not null default 'ai_generated' check (
    source_type in ('ai_generated', 'manual', 'licensed')
  ),
  ...
  points_correct_with_ai int not null default 6,
...
create index if not exists questions_exam_type_lesson_topic_idx
  on public.questions (exam_type, lesson, topic);

create index if not exists questions_active_idx
  on public.questions (exam_type, is_active, is_approved);
...
create table if not exists public.question_attempts (
  ...
  exam_type text not null check (exam_type in ('YKS', 'LGS', 'KPSS', 'ALES')),
  question_id uuid not null references public.questions (id) on delete cascade,
  selected_option_key text check (selected_option_key in ('A', 'B', 'C', 'D', 'E')),
...
create index if not exists mock_attempts_user_exam_completed_idx
  on public.mock_attempts (user_id, exam_type, completed_at desc);
...
create table if not exists public.leaderboard_stats (
  ...
  rank_position int,
  updated_at timestamp with time zone not null default now(),
  unique (user_id, exam_type, period_type, period_start)
);

create index if not exists leaderboard_stats_exam_period_rank_idx
  on public.leaderboard_stats (exam_type, period_type, period_start, total_points desc, total_net desc);
...
begin
  ...
  new.exam_type := question_record.exam_type;
  penalty_divisor := public.get_wrong_penalty_divisor(question_record.exam_type);
  ...
  if new.is_blank then
    ...
    new.points_awarded := case
      when new.used_ai_help then question_record.points_correct_with_ai
      else question_record.points_correct
    end;
    new.net_delta := 1.00;
  else
    new.points_awarded := 0;
...
declare
  ...
  base_points int;
begin
  penalty_divisor := public.get_wrong_penalty_divisor(new.exam_type);
...
begin
  ...
  loop
    ...
    select
      ...
      current_period_end,
      coalesce(question_data.total_points, 0) + coalesce(mock_data.total_points, 0) as total_points,
      coalesce(question_data.total_net, 0) + coalesce(mock_data.total_net, 0) as total_net,
    ...
    on conflict (user_id, exam_type, period_type, period_start)
    do update set
...
begin
  ...
  perform public.refresh_leaderboard_stats_for_user_exam(target_user_id, target_exam_type, now());
  ...
  perform public.update_rankings_for_exam_period(target_exam_type, 'weekly', weekly_start);
  perform public.update_rankings_for_exam_period(target_exam_type, 'monthly', monthly_start);
  perform public.update_rankings_for_exam_period(target_exam_type, 'all_time', all_time_start);
...
using (
  exists (
    ...
    where questions.id = question_options.question_id
      ...
      and questions.is_approved = true
  )
);
